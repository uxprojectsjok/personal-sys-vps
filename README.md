...
> [!NOTE]
> **Third-party services:** This codebase integrates independent third-party services (Anthropic, ElevenLabs, Reown, Pinata, Polygon, Zapier). I am not affiliated with, endorsed by, or a partner of any of them — their use reflects my own technical choices, not a recommendation. Full disclaimer: [Legal](#legal).
...
| **Multi-Hoster** | Multiple souls on one VPS. No soul lock. Suitable for families, teams, or soul hosting services. Agent Runner not available on a fresh Multi-Hoster install — `init.sh` skips setup entirely, since it runs as root and would have access to all souls on the node. Not re-checked at runtime, so switching an existing node to Multi-Hoster later does not retroactively remove it. |
...
Current release fingerprint (v1.3.14): 1652a845b29f3f79
...
Copyright © 2026 [UX-Projects Jan-Oliver Karo](https://uxprojects-jok.com), Marburg, Germany

"SaveYourSoul" and "SYS" are trademarks of [UX-Projects Jan-Oliver Karo](https://uxprojects-jok.com). See [NOTICE](NOTICE) for trademark and attribution requirements.
