...
// Parameter anchor_count:0/Wissenswert:0, obwohl die echte Genesis
...
export function register(server, token, soulId) {
  server.tool(
    'soul_chain_metrics',
    'Liest die Genesis-Chain-Metriken der Soul: Chain Age in Tagen (primär) und Polygon-Blöcken (on-chain Beleg), Knowledge Score (altersgewichteter KB-Wert aus Größe × Alter — kein Zähler diskreter Einheiten), Anchor-Anzahl, Genesis-Block und -Zeitstempel. Gibt sofort Auskunft über den on-chain Wissenswert ohne den vollen Maturity-Report zu laden.',
    {},
    async () => {
      let metrics;
      try {
      ...
      const result = {
        ...
        // days zuerst — primäre, semantische Alterseinheit; blocks ist der
        // sekundäre, on-chain überprüfbare Beleg (siehe docs/spec/
        // chain-metrics-terminology.md für die Begründung).
        chain_age: {
