...
// ── Peer-Auflösung + Inbox-Relay für gewirte Souls ──────────────────────────
// Macht @peer (peer_send/peer_inbox) nicht nur für die alten 1:1-Connections
// nutzbar, sondern automatisch für jede Soul, die bei einem Gatekeeper gewired
// ist — mit allen Geschwister-Spokes desselben Gatekeepers UND dem Gatekeeper
// selbst, auch cross-node (die Spoke kennt den node_url ihres Gatekeepers
// bereits aus wired_to.json, siehe /mcp/discover/wire-out).
// Sicherheitsmodell: eine Spoke bekommt NIE die Tokens ihrer Geschwister zu
// sehen — die hält ausschließlich der Gatekeeper in wired_souls.json.
// Nachrichten lesen läuft daher über den Relay unten, nie direkt Spoke-zu-
// Spoke. Senden braucht dagegen keinen Relay: peer_send schreibt nur in die
// eigene sys.md (siehe peer_send.mjs) — dafür reicht die Namensauflösung.

// true wenn callerSoulId (ggf. cross-node über callerNodeUrl) selbst der
// Gatekeeper ist ODER eine bei ihm akzeptierte, gewirte Soul.
async function verifyGatekeeperOrSpoke(gatekeeperSoulId, callerSoulId, callerNodeUrl) {
  if (callerSoulId === gatekeeperSoulId && !callerNodeUrl) return true;
  const wiredAll = await loadWired(gatekeeperSoulId);
  const entry = wiredAll[wireKey(callerSoulId, callerNodeUrl)];
  return !!entry && (!entry.status || entry.status === 'accepted');
}

app.get('/mcp/discover/gatekeeper/peers', async (req, res) => {
  const parsed = parseOwnCertBearer(req);
  if (!parsed) return res.status(401).json({ error: 'soul_cert_required' });
  const { soulId: callerSoulId, cert } = parsed;
  const callerNodeUrl = typeof req.query.caller_node_url === 'string' && req.query.caller_node_url.trim()
    ? req.query.caller_node_url.trim().replace(/\/$/, '') : null;
  if (!(await verifyPeerCert(callerSoulId, cert, callerNodeUrl))) {
    return res.status(401).json({ error: 'invalid_cert' });
  }

  const gatekeeperSoulId = req.query.gatekeeper_soul_id;
  if (!gatekeeperSoulId) return res.status(400).json({ error: 'gatekeeper_soul_id erforderlich' });
  const gkExists = await stat(`${SOULS_DIR}${gatekeeperSoulId}`).then(s => s.isDirectory()).catch(() => false);
  if (!gkExists) return res.status(404).json({ error: 'gatekeeper_not_found' });

  if (!(await verifyGatekeeperOrSpoke(gatekeeperSoulId, callerSoulId, callerNodeUrl))) {
    return res.status(403).json({ error: 'not_wired', message: 'Nur gewirte Souls (oder der Gatekeeper selbst) sehen diese Liste.' });
  }

  const { wired } = await loadAcceptedWired(gatekeeperSoulId);
  const peers = Object.values(wired)
    .filter(e => e.soul_id !== callerSoulId)
    .map(e => ({ soul_id: e.soul_id, name: e.name, node_url: e.node_url || BASE_URL }));

  // Der Gatekeeper selbst ist immer mit erreichbar — außer der Aufrufer IST der Gatekeeper.
  if (callerSoulId !== gatekeeperSoulId) {
    const gkCtx = await loadCtx(gatekeeperSoulId).catch(() => null);
    peers.unshift({ soul_id: gatekeeperSoulId, name: gkCtx?.name || gatekeeperSoulId, node_url: BASE_URL, is_gatekeeper: true });
  }

  res.json({ ok: true, gatekeeper_soul_id: gatekeeperSoulId, peers });
});

app.post('/mcp/discover/gatekeeper/peer-inbox-relay', async (req, res) => {
  const parsed = parseOwnCertBearer(req);
  if (!parsed) return res.status(401).json({ error: 'soul_cert_required' });
  const { soulId: callerSoulId, cert } = parsed;
  const { gatekeeper_soul_id: gatekeeperSoulId, caller_node_url } = req.body || {};
  const callerNodeUrl = typeof caller_node_url === 'string' && caller_node_url.trim()
    ? caller_node_url.trim().replace(/\/$/, '') : null;
  if (!(await verifyPeerCert(callerSoulId, cert, callerNodeUrl))) {
    return res.status(401).json({ error: 'invalid_cert' });
  }
  if (!gatekeeperSoulId) return res.status(400).json({ error: 'gatekeeper_soul_id erforderlich' });
  if (!(await verifyGatekeeperOrSpoke(gatekeeperSoulId, callerSoulId, callerNodeUrl))) {
    return res.status(403).json({ error: 'not_wired' });
  }

  const { wired } = await loadAcceptedWired(gatekeeperSoulId);
  const messages = [];
  await Promise.all(Object.values(wired).map(async (entry) => {
    if (entry.soul_id === callerSoulId || !entry.permissions?.soul) return;
    try {
      ...
      for (const m of parseSocialMessages(md)) {
        if (m.to === 'peer' || m.to === callerSoulId) {
          messages.push({
            ...m,
...
// jeden Gatekeeper, bei dem diese Soul selbst gewired ist (same-node + cross-
// node). Frontend-Konsum für GatekeeperPanel.vue — läuft server-seitig, weil
// ein direkter Browser-fetch() auf einen fremden Node-Origin an der strikten
// connect-src-CSP scheitern würde (gleicher Grund wie bei /mcp/discover/
// wire-out).
app.get('/mcp/discover/my-peers', async (req, res) => {
  const parsed = parseOwnCertBearer(req);
  if (!parsed) return res.status(401).json({ error: 'soul_cert_required' });
  if (!(await verifyPeerCert(parsed.soulId, parsed.cert, null))) {
    return res.status(401).json({ error: 'invalid_cert' });
  ...
  try {
    const peers = await resolveGatekeeperPeers(parsed.soulId, `${parsed.soulId}.${parsed.cert}`);
