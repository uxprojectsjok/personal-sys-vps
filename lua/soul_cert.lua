      ...
      if rnd then
        ...
        if bytes_key and bytes_adm and #bytes_key == 32 and #bytes_adm == 32 then
          ...
          if saf then
            ...
            -- Invite-Token nach erfolgreicher Registrierung rotieren. Nutzt
            -- das oben (Zeile 78) bereits eingelesene master_data statt
            -- master.json hier ein zweites Mal unabhängig neu zu lesen — ein
            -- eigenständiger Re-Read+Write öffnete ein Race-Fenster: eine
            -- zwischen dem ersten Read und diesem zweiten Read abgeschlossene,
            -- gleichzeitige Anfrage auf master.json (z.B. der first_soul_id-
            -- Migrationsblock oben, für eine ANDERE gerade neu registrierende
            -- Soul) wurde von diesem Write wieder überschrieben, weil er auf
            -- einem älteren, unabhängig gelesenen Snapshot basierte. Live
            -- beobachtet: first_soul_id verschwand exakt im Zeitfenster einer
            -- Neuregistrierung.
            ...
            if rnd_inv then
              ...
              if inv_bytes and #inv_bytes == 16 then
                ...
                master_data.invite_token = inv_hex
                local mpath_inv = get_master_path()
                local inv_wf = io.open(mpath_inv, "w")
                if inv_wf then
                  inv_wf:write(cjson.encode(master_data)); inv_wf:close()
                ...
                if mpath_inv ~= MASTER_PATH_GLOBAL then
                  ...
                  if g_inv_wf then
                    g_inv_wf:write(cjson.encode(master_data)); g_inv_wf:close()
                    os.execute("chmod 600 " .. MASTER_PATH_GLOBAL)
                    os.execute("chown www-data:www-data " .. MASTER_PATH_GLOBAL .. " 2>/dev/null || true")
                  end
                end
