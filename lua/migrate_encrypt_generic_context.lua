-- migrate_encrypt_generic_context.lua
-- One-time migration: encrypts every remaining plaintext file in
-- vault/context/ for every soul with cipher_mode=ciphered + a vault_key.
-- Covers old files created via context_write (or manual upload) before
-- encryption was added to that write path -- arbitrary names, not just the
-- specifically-named files already covered by migrate_encrypt_mind_earnings.lua
-- and migrate_encrypt_vault_media.lua.
-- Excludes shopping.md and prompts.md, which are deliberately never
-- encrypted (see README "Encrypted at rest, server-managed").
-- Idempotent -- files already starting with the SYS\x01 magic are skipped.
-- Not wired into any nginx location; run manually:
--   resty lua/migrate_encrypt_generic_context.lua
...
local EXCLUDE   = { ["shopping.md"] = true, ["prompts.md"] = true }
...
local function migrate_soul(soul_id)
  ...
  local results = {}
  for _, name in ipairs(list_dir(context_dir)) do
    if not EXCLUDE[name:lower()] and not name:match("ownagent%.md$") then
      local path = context_dir .. "/" .. name
      local status = encrypt_file_if_plaintext(path, vault_key_hex)
      results[#results + 1] = name .. ": " .. status
    end
  end
...
print("Migriere generische vault/context/-Dateien für " .. #souls .. " Soul(s)…")
