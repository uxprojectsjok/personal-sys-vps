...
if imf then
  ...
  -- ── GitHub-Backup: income.md nach jedem Payment ins private Repo pushen ──────
  -- Läuft im Hintergrund nach der Response → Payment-Latenz unverändert
  local _income_file = income_file
  local _soul_id     = soul_id
  local _ts          = new_entry.redeemed_at
  local ok_t = ngx.timer.at(0, function()
    -- GitHub-Token aus git-Remote-URL lesen
    local p = io.popen("git -C /var/www/SaveYourSoul_init remote get-url origin 2>/dev/null")
    if not p then return end
    local remote_url = p:read("*a"); p:close()
    local gh_token = remote_url:match("(ghp_[%w]+)")
    if not gh_token or gh_token == "" then return end

    local repo_path = remote_url:match("github%.com/([^%s]+)%.git") or
                      remote_url:match("github%.com/([^%s\n]+)")
    if not repo_path then return end

    -- income.md lesen
    local f2 = io.open(_income_file, "r")
    if not f2 then return end
    local content = f2:read("*a"); f2:close()
    local encoded = ngx.encode_base64(content)

    local api_file_path = "data/income_" .. _soul_id .. ".md"
    local api_url = "https://api.github.com/repos/" .. repo_path .. "/contents/" .. api_file_path

    local httpc_gh = require("resty.http").new()
    httpc_gh:set_timeout(12000)

    -- Aktuellen SHA abrufen (für Update statt Create)
    local get_r = httpc_gh:request_uri(api_url, {
      method  = "GET",
      headers = {
        ["Authorization"] = "token " .. gh_token,
        ["User-Agent"]    = "soul-pay/1.0",
        ["Accept"]        = "application/vnd.github.v3+json",
      },
    })
    local sha = nil
    if get_r and get_r.status == 200 then
      local ok_g, gd = pcall(require("cjson.safe").decode, get_r.body or "")
      if ok_g and type(gd) == "table" then sha = gd.sha end
    end

    -- PUT: Create oder Update
    local put_body = { message = "income: payment " .. _ts, content = encoded }
    if sha then put_body.sha = sha end
    httpc_gh:request_uri(api_url, {
      method  = "PUT",
      body    = require("cjson.safe").encode(put_body),
      headers = {
        ["Authorization"] = "token " .. gh_token,
        ["Content-Type"]  = "application/json",
        ["User-Agent"]    = "soul-pay/1.0",
  ...
  if not ok_t then
    ngx.log(ngx.WARN, "soul_pay: github backup timer failed to schedule")
  end
end
