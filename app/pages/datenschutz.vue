<template>
  <div class="legal-page">
    <nav class="l-nav">
      ...
      <div class="center"><span class="page-title">{{ t('datenschutz.pageTitle') }}</span></div>
      <div class="nav-end">
    ...
    <main class="content">
      ...
      <h2>{{ t('datenschutz.s2h2') }}</h2>

      <h3>{{ t('datenschutz.s2VaultH3') }}</h3>
      <p v-html="t('datenschutz.s2VaultContent')"></p>

      <h3>{{ t('datenschutz.s2HealthH3') }}</h3>
      <p v-html="t('datenschutz.s2HealthContent')"></p>

      <h3>{{ t('datenschutz.s2PaymentH3') }}</h3>
      <p v-html="t('datenschutz.s2PaymentContent')"></p>

      <h3>{{ t('datenschutz.s2PeerH3') }}</h3>
      <p v-html="t('datenschutz.s2PeerContent')"></p>
      ...
      <h2>{{ t('datenschutz.s4h2') }}</h2>
      <p>{{ t('datenschutz.s4Content') }}</p>

      <h2>{{ t('datenschutz.s5h2') }}</h2>
      <p>{{ t('datenschutz.s5Content') }}</p>
      ...
      <div class="rights-list">
        <div class="rights-row"><span class="rights-label">{{ t('datenschutz.rightAccess') }}</span><span>{{ t('datenschutz.rightAccessDesc') }}</span></div>
        <div class="rights-row"><span class="rights-label">{{ t('datenschutz.rightRect') }}</span><span>{{ t('datenschutz.rightRectDesc') }}</span></div>
        <div class="rights-row"><span class="rights-label">{{ t('datenschutz.rightErase') }}</span><span>{{ t('datenschutz.rightEraseDesc') }}</span></div>
        <div class="rights-row"><span class="rights-label">{{ t('datenschutz.rightRestrict') }}</span><span>{{ t('datenschutz.rightRestrictDesc') }}</span></div>
        <div class="rights-row"><span class="rights-label">{{ t('datenschutz.rightObject') }}</span><span>{{ t('datenschutz.rightObjectDesc') }}</span></div>
      </div>
      <p v-html="t('datenschutz.s7Contact')"></p>

      <h2>{{ t('datenschutz.s8h2') }}</h2>
      <p v-html="t('datenschutz.s8Content')"></p>

      <h2>{{ t('datenschutz.s9h2') }}</h2>
      <p>{{ t('datenschutz.s9Content') }}</p>

      <p class="fine">{{ t('datenschutz.stand') }}</p>
    </main>
  </div>
</template>
...
useSeoMeta({ title: 'Datenschutz – SYS', robots: 'noindex' })
...
.content h3 { font-family: var(--sans); font-size: 16px; font-weight: 600; color: var(--fg); margin: 24px 0 8px; }
...
.content :deep(code) { font-family: var(--mono); font-size: 14px; background: var(--surface-2); padding: 1px 6px; border-radius: 4px; }
.content :deep(a) { color: var(--accent-bright); text-decoration: none; }
.content :deep(a:hover) { text-decoration: underline; }
.content :deep(.i-name) { font-weight: 600; }
.content :deep(.i-sub) { font-size: 13px; color: var(--fg-3); }
...
@media (max-width: 640px) {
  ...
  .center { display: none; }
  .rights-row { flex-direction: column; gap: 2px; }
}
</style>
