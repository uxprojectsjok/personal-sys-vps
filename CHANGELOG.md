...
**Fixed: identity verification (`/verify`) was broken end-to-end on a fresh node — face check always crashed, voice HQ always failed instantly, and both showed the same uninformative generic error, all from unrelated causes.**

- **`module 'resty.http' not found`, 500, on every handler that calls an external API.** `lua-resty-http` isn't bundled with OpenResty and is `opm`-only, yet ~50 handlers (`verify_face_check`, `verify_voice_hq_check`, `vision_analyze`, `translate`, `web_search`, the x402 payment handlers, `soul_register`, peer-to-peer connections, ...) `require()` it. None of them had ever been exercised on this particular node, so the gap went unnoticed until `/verify` surfaced it. **Fixed:** install it (`opm get pintsized/lua-resty-http`); documented as a real runtime dependency in `README.md` (previously said "no runtime dependencies beyond OpenResty," which was no longer true).
- **`verify.vue`'s `captureFace()` read `d.reason` from the face-check response; the Lua handler returns the message under `d.message`.** `d.reason` was always `undefined`, so every failure — a real no-match, a missing API key, a missing profile image, the `resty.http` crash above — showed the identical generic "Kein Gesichts-Match.", discarding whatever specific reason the server actually gave. **Fixed:** read the right field; also added `message` to the two `verify_face_check.lua` error paths that had none (`anthropic_key_missing`, `profile_image_not_found`), and surfaced `getUserMedia`'s exception name (`NotAllowedError`/`NotReadableError`/...) instead of swallowing it, so a desktop camera failure is diagnosable without opening devtools.
- **Voice HQ verification failed instantly on every attempt, no request ever reaching `/api/verify/voice-hq-check` — looked like a liveness/audio problem, wasn't.** `/api/vault/audio`'s `active_url` (and 8 other absolute-URL call sites across `connect_create.lua`, `peer_connect.lua`, `peer_verify.lua`, `soul_connections.lua` ×3, `soul_pay_x402.lua`, `webhook.lua`, `webhook_mnemonic.lua`) were built from `ngx.var.scheme` — the scheme of the *local* connection. Behind a reverse tunnel where TLS terminates upstream (e.g. a self-hosted `frp` tunnel) that's `http` while the page itself is `https`; the resulting `http://` URL got silently mixed-content-blocked by the browser — no network request, no console error, nothing to grep in `access.log`. `X-Forwarded-Proto` looked like the fix but wasn't reliable either: some tunnel software rewrites it to its own view of the last hop regardless of what an upstream proxy set it to. **Fixed:** `config_reader.request_scheme()` now reads a static `/var/lib/sys/config/public_scheme` file (the operator's actual public URL scheme, known correct by construction) first, falling back to `X-Forwarded-Proto` then `ngx.var.scheme` for setups that don't provide it. Node operators outside the `SaveYourSoul_local_installer` flow: write `https` (or `http`) to that file yourself if any absolute URL your node returns is being silently dropped by browsers.

**Migration required:**
- `opm get pintsized/lua-resty-http` on any node not yet running it (check: does `/var/log/sys/error.log` show `module 'resty.http' not found`?).
- `echo -n https > /var/lib/sys/config/public_scheme` (or `http`, matching the node's actual public URL) if TLS terminates somewhere other than this OpenResty instance.
- `update.sh` (Lua + frontend redeploy) for the rest.

**Confirmed live end-to-end on node1** 2026-09-03: fingerprint, face, and voice-HQ verification, and `soul_read` via MCP, all working after these fixes — none did before.
